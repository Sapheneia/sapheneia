...
class DataProcessor:
    ...
    def load_csv_data(
        self, csv_file_path: str, data_definition: str | dict[str, str]
    ) -> pd.DataFrame:
        """
        ...
        if isinstance(data_definition, str):
            with open(data_definition) as f:
    ...
    def _validate_data_definition(self) -> None:
        ...
        if len(target_columns) > 1:
            logger.warning(
                f"Multiple target columns found: {target_columns}. Using first one for univariate forecasting."
            )
        ...
        if missing_columns:
            raise ValueError(
                f"Columns defined in data_definition but missing from CSV: {missing_columns}"
            )
    ...
    def prepare_forecast_data(
        ...
        if target_column is None:
            target_columns = [
                col for col, dtype in self.data_definition.items() if dtype == "target"
            ]
            if not target_columns:
        ...
        logger.info("Target data preparation:")
        ...
        logger.info("✅ Prepared forecast data:")
        logger.info(f"  Target inputs length: {len(target_inputs)}")
        logger.info(f"  Target range: {min(target_inputs):.2f} - {max(target_inputs):.2f}")
    ...
    def _prepare_covariates(
        self, data: pd.DataFrame, context_len: int, horizon_len: int
        ...
        logger.info("Covariate preparation debug:")
        ...
        for column, data_type in self.data_definition.items():
            ...
            if data_type == "dynamic_numerical":
                ...
                if len(data) < context_len:
                    logger.warning(
                        f"Insufficient data for dynamic covariate '{column}': need {context_len} for context, have {len(data)}"
                    )
                    continue
                ...
                logger.info(
                    f"  - Context period: {len(context_values)} values (last {context_len} periods)"
                )
                logger.info(
                    f"  - Horizon period: {len(horizon_values)} values (padded with {last_value})"
                )
    ...
    def create_sample_data_definition(self, output_path: str) -> None:
        ...
        print("Sample data definition structure:")
        print(json.dumps(sample_definition, indent=2))


def prepare_visualization_data(
    processed_data: pd.DataFrame,
    target_inputs: list[float] | list[list[float]],
    target_column: str,
    context_len: int,
    horizon_len: int,
    ...
    if target_inputs_flat:
        ...
        logger.info("Using target_inputs for historical data to ensure forecasting alignment")
    else:
        # Fallback to data-based extraction if target_inputs not available
        if len(df) >= context_len_effective:
            historical_slice = (
                df[target_column].iloc[-context_len_effective:].astype(float).tolist()
            )
            dates_historical = df["date"].iloc[-context_len_effective:].tolist()
        else:
        ...
        logger.info("Using data-based extraction for historical data")

    logger.info("Visualization data preparation:")
    ...
    logger.info(
        f"  - Historical data range: {min(historical_slice) if historical_slice else 'N/A'} to {max(historical_slice) if historical_slice else 'N/A'}"
    )
    if dates_historical:
    ...
    if len(data_for_future_extraction) > context_len_effective and dates_historical:
        ...
        if len(future_data) > 0:
            ...
            logger.info("Actual future values extracted:")
            ...
            if future_slice and dates_future:
                logger.info(
                    f"  - Future values range: {min(future_slice):.4f} to {max(future_slice):.4f}"
                )
    ...
    if len(dates_future) < horizon_len:
        ...
        inferred_delta: pd.Timedelta | None = None
        ...
        step = (
            inferred_delta
            if isinstance(inferred_delta, pd.Timedelta) and inferred_delta != pd.Timedelta(0)
    ...
    visualization_data = {
        ...
        "dates_historical": [
            d.isoformat() if hasattr(d, "isoformat") else str(d) for d in dates_historical
        ],
        "dates_future": [
            d.isoformat() if hasattr(d, "isoformat") else str(d) for d in dates_future
        ],
        "target_name": target_column,
