...
if TYPE_CHECKING:
    # Imported only by type checkers / IDEs to preserve return-type information
    # on initialize_timesfm_model(). The actual runtime imports happen inside
    # the function body (circular-import avoidance).
...
class TimesFMModel:
    ...
    def __init__(
        ...
        logger.info("TimesFM Model Configuration:")
    ...
    def load_model(self) -> timesfm.TimesFm:
        ...
        try:
            ...
            if self.checkpoint:
                ...
                checkpoint_config = timesfm.TimesFmCheckpoint(huggingface_repo_id=self.checkpoint)
                logger.info(f"Loading from HuggingFace: {self.checkpoint}")
            else:
                # Load from local path
                checkpoint_config = timesfm.TimesFmCheckpoint(path=self.local_model_path)
                logger.info(f"Loading from local path: {self.local_model_path}")

            # Initialize model
            self.model = timesfm.TimesFm(hparams=hparams, checkpoint=checkpoint_config)
    ...
    def _validate_model(self) -> None:
        ...
        try:
            ...
            if forecast_array.shape != expected_shape:
                raise ValueError(
                    f"Unexpected forecast shape: {forecast_array.shape}, expected: {expected_shape}"
                )
            ...
            if hasattr(self.model, "experimental_quantile_forecast"):
                ...
                self.model.experimental_quantile_forecast(inputs=test_inputs, freq=test_freq)
