...
class Forecaster:
    ...
    def forecast(
        ...
        dynamic_categorical_covariates: dict[str, list[list[str]]] | None = None,
        static_numerical_covariates: dict[str, list[float]] | None = None,
        static_categorical_covariates: dict[str, list[str]] | None = None,
        ...
        try:
            if use_covariates and any(
                ...
                logger.info(
                    f"Inputs shape: {[len(x) for x in inputs] if isinstance(inputs[0], list) else len(inputs)}"
                )
                ...
                if isinstance(covariates_result, tuple) and len(covariates_result) == 2:
                    ...
                    logger.info("✅ Covariates forecasting completed.")
                    ...
                    if quantile_forecast.ndim == 2 and (
                        quantile_forecast.shape[0] == 1 or quantile_forecast.shape[1] == 1
                    ):
                        logger.warning(
                            "⚠️ Covariates forecasting returned insufficient quantiles, falling back to basic forecast for quantiles"
                        )
                        # Get quantiles from basic forecast method
                        _, quantile_forecast = self.model.forecast(
                            inputs=inputs_norm, freq=freq_norm
                        )
                        quantile_forecast = np.array(quantile_forecast)
                        logger.info(
                            f"✅ Basic forecast quantiles obtained. Shape: {quantile_forecast.shape}"
                        )
                    else:
                    ...
                    logger.warning(
                        "⚠️ Covariates forecasting didn't return quantiles, getting them separately"
                    )
                ...
                logger.info("✅ Basic forecasting completed.")
    ...
    def _validate_covariates(
        ...
        dynamic_numerical: dict | None,
        dynamic_categorical: dict | None,
        static_numerical: dict | None,
        static_categorical: dict | None,
    ) -> None:
                ...
                for name, data in cov_data.items():
                    if isinstance(data[0], (list, np.ndarray)):
                        ...
                        if len(data) != num_series:
                            raise ValueError(
                                f"Dynamic covariate '{name}' has {len(data)} series, expected {num_series}"
                            )
                    else:
                        # Static covariates
                        if len(data) != num_series:
                            raise ValueError(
                                f"Static covariate '{name}' has {len(data)} values, expected {num_series}"
                            )
...
def run_forecast(
    ...
    try:
        ...
        if use_covariates and covariates:
            ...
            point_forecast, quantile_forecast = forecaster.forecast(
                inputs=target_inputs, freq=freq, use_covariates=False
            )
        ...
        if np.any(np.isnan(point_forecast)):
            logger.error(
                f"❌ NaN values detected in point_forecast: {np.isnan(point_forecast).sum()} out of {point_forecast.size}"
            )
            logger.error(f"Point forecast values: {point_forecast}")
            raise ValueError(
                "Forecasting produced NaN values in point forecast. This may be due to insufficient data or model issues."
            )

        if np.any(np.isnan(quantile_forecast)):
            logger.error(
                f"❌ NaN values detected in quantile_forecast: {np.isnan(quantile_forecast).sum()} out of {quantile_forecast.size}"
            )
            logger.error(f"Quantile forecast shape: {quantile_forecast.shape}")
            raise ValueError(
                "Forecasting produced NaN values in quantile forecast. This may be due to insufficient data or model issues."
            )
        ...
        logger.info("✅ Centralized forecast completed successfully!")
        ...
        logger.info(
            f"   Point forecast range: {np.min(point_forecast):.2f} to {np.max(point_forecast):.2f}"
        )
...
def process_quantile_bands(
    quantile_forecast: np.ndarray, selected_indices: list[int] = None
) -> dict[str, Any]:
    """
    ...
    logger.info(
        f"Input quantile_forecast shape: {quantile_forecast.shape if hasattr(quantile_forecast, 'shape') else 'N/A'}"
    )
    ...
    try:
        ...
        logger.info(
            f"📊 Note: Index 0 is legacy mean forecast, using indices 1-{num_quantiles - 1} for actual quantiles"
        )

        # Check if we have enough quantiles for band creation (need at least 3 total: 0=legacy, 1=Q10, 2=Q20)
        if num_quantiles < 3:
            logger.warning(
                f"Not enough quantiles for band creation. Have {num_quantiles}, need at least 3"
            )
            return {}

        # Filter selected indices to valid range (skip index 0)
        valid_indices = [
            idx for idx in selected_indices if 1 <= idx < num_quantiles
        ]  # Skip index 0
        ...
        for i in range(len(valid_indices) - 1):
            ...
            logger.info(
                f"   Band {band_count}: {band_label} - Lower: {len(lower_quantile)}, Upper: {len(upper_quantile)}"
            )
            band_count += 1
