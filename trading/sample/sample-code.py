...
class TradingStrategy:
    ...
    def calculate_return_signal(params: Dict) -> Dict:
        ...
        elif position_sizing == 'proportional' or position_sizing == PositionSizing.PROPORTIONAL.value:
            # Scale position by expected return magnitude
            # e.g., 10% expected return → 10x execution_size, 1% → 1x execution_size
            position_size = execution_size * abs(expected_return) * 100  # *100 to convert decimal to %
    ...
    def calculate_quantile_signal(params: Dict) -> Dict:
        ...
        if signal_action == 'buy':
            ...
            if position_sizing == 'normalized' or position_sizing == PositionSizing.NORMALIZED.value:
    ...
    def _calculate_threshold(
        ...
        if threshold_type == 'absolute' or threshold_type == ThresholdType.ABSOLUTE.value:
            ...
            if open_history is None or high_history is None or low_history is None or close_history is None:
                # Fallback to absolute
                return threshold_value
            
            atr = TradingStrategy._calculate_atr(
                open_history, high_history, low_history, close_history, window_history, min_history_length
            )
    ...
    def _calculate_atr(
        ...
        if len(high_history) < min_history_length or len(low_history) < min_history_length or len(close_history) < min_history_length:
            return 0.0
    ...
    @staticmethod
    def get_portfolio_value(current_position: float, current_price: float, available_cash: float) -> float:
        """Calculate total portfolio value (cash + position value)"""
        position_value = current_position * current_price
        return available_cash + position_value
    
    @staticmethod
    def get_portfolio_return(current_position: float, current_price: float, available_cash: float, initial_capital: float) -> float:
        """Calculate portfolio return relative to initial capital"""
        if initial_capital == 0:
            return 0.0
        
        current_value = TradingStrategy.get_portfolio_value(current_position, current_price, available_cash)
