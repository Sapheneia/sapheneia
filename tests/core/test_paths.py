...
class TestPathTypeHandling:
    ...
    def test_empty_string_returns_uploads_dir(self):
        """Test that empty string resolves to uploads directory."""
        # Empty string should be treated as a bare filename and resolve to uploads
        result = normalize_data_path("")
        # The function should either raise an error or return the uploads dir
        # Based on implementation, it goes to uploads directory
        assert "uploads" in str(result) or result == Path("")
